...
public class DoublyLinkedList<T> {
    ...
    public T removeLastOccurrence(T data) {
        ...
        if (head.getData().equals(data) || head.getData() == data) {
            DoublyLinkedListNode<T> tempNode = head;
            head = head.getNext();
            head.setPrevious(null);
            size--;
            return tempNode.getData();
        } else if (tail.getData().equals(data) || tail.getData() == data) {
            DoublyLinkedListNode<T> tempNode = tail;
            ...
            DoublyLinkedListNode<T> curNode = head;
            int check = 0;
            while (!(curNode.getData().equals(data)) || curNode.getData() != data) {
                curNode = curNode.getNext();
            }
            if (curNode.getData().equals(data) || curNode.getData() == data) {
                curNode.getPrevious().setNext(curNode.getNext());
                curNode.getNext().setPrevious(curNode.getPrevious());
                size--;
                return curNode.getData();
            } else {
                throw new NoSuchElementException("Input data is NOT in the list.");
            }
        }
    }
